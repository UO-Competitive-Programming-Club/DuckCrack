...
public:
    ...
    void solve(vector<vector<char>>& grid) {
        ...
        /* top border */
        for (int col = 0; col < w; col++) { PaintColorRegion(grid, {0, col}, 'O', '@'); }
        /* bottom border */
        for (int col = 0; col < w; col++) { PaintColorRegion(grid, {h - 1, col}, 'O', '@'); }
        /* left border */
        for (int row = 0; row < h; row++) { PaintColorRegion(grid, {row, 0}, 'O', '@'); }
        /* right border */
        for (int row = 0; row < h; row++) { PaintColorRegion(grid, {row, w - 1}, 'O', '@'); }
